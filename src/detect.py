...
def detect_drone_video(model_path, video_path, output_path=None, conf_threshold=0.25, 
                       img_size=640, frame_skip=3, device=None):
    """
    Phát hiện drone trong video với frame skipping và resize
...
def detect_drone_image(model_path, image_path, output_path=None, conf_threshold=0.25, img_size=640):
    """
    Phát hiện drone trong ảnh với resize
    ...
    cv2.imshow('Drone Detection', annotated_image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
...
if __name__ == "__main__":
    ...
    # Cấu hình
    VIDEO_PATH = "demo_videos/demo.mp4"
